...
if 'output' in config['LEARNING']:
    posttrained_model = BertForMultipleChoice.from_pretrained(config['LEARNING']['output']) # posttrained model
else:
    posttrained_model = BertForMultipleChoice.from_pretrained('bert-base-uncased')
...
for run_ctr in range(nr_runs):
    ...
    #     dbenv=env)
    posttrained_env = MultiDocBart(
        docs=docs, max_length=max_length, mask_params=None, hint_order=hint_order,
        dbms=dbms, benchmark=bench, hardware={'memory':memory, 'disk':disk, 'cores':cores},
        hints_per_episode=nr_hints, nr_evals=nr_evals, scale_perf=p_scaling,
        scale_asg=a_scaling, objective=objective, rec_path=None, use_recs=None,
        posttrained_model=posttrained_model, dbenv=env
    )
    posttrained_env.reset()
    # unsupervised_env = GymEnvironment(unsupervised_env, device=device)
    
    # Initialize agents
    model = A2C(
        'MlpPolicy', posttrained_env, 
